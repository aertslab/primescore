...
def k_sliding_windows2fasta_insertion( chr, start_indel, indel, sw_size ):
    ...
    for i in range( 0, sw_size+1,shift ):
        ...
        ref_nuc = indel.split(":")[0]
        mut_nuc =  indel.split(":")[1]
        ...
        if fasta_sequence[mut_start_local_coord] == ref_nuc:
            ...
            fasta_sequence_insertion = fasta_sequence_new[:insertion_start] + iserting_sequence + fasta_sequence_new[insertion_start:]
            if regionID not in regionID_mut_fasta_dict:
                regionID_mut_fasta_dict[regionID] = fasta_sequence_insertion
        else:
            print "Reference nucleotide differs from hg19 version", mut_start_chr, mut_start ,ref_nuc, mut_nuc
            fasta_sequence_insertion = fasta_sequence.tostring()
            insertion_start = mut_start_local_coord+1
            fasta_sequence_insertion = fasta_sequence_new[:insertion_start] + iserting_sequence + fasta_sequence_new[insertion_start:]
            if regionID not in regionID_mut_fasta_dict:
                regionID_mut_fasta_dict[regionID] = fasta_sequence_insertion
...
if __name__ == '__main__':
    ...
    parser.add_option("-b", "--bed_file", action = "store", type = "string", dest = "bed_file", help = 'file with mutations (insertions)')
    ...
    num_positions_to_score=len(coord_muttype_dict.keys())
    ...
    print "Number of positions to score", num_positions_to_score
    if num_positions_to_score==0 or num_positions_to_score<0:
        print "check format of input file"
        print "Make shure the format is like this:"
        print "chr10	100028287	G	GCA"
        sys.exit(1)
    ...
    for indel in coord_muttype_dict:
        if indel not in scored_already_insertions:
            ...
            regionID_fasta_dict_ref, regionID_mut_fasta_dict = k_sliding_windows2fasta_insertion( chrom, start, indel_descr, SLIDING_WINDOW_SIZE )
